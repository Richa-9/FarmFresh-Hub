...
router.post(
  ...
  [
    ...
    check("password")
      ...
      .isAlphanumeric()
      .withMessage("Password can only contain alphabets and numbers"),
...
router.post("/addtocart/:id", authenticate, async function (req, res) {
  try {
    ...
    if (userInfo) {
      ...
      if (flag) {
        // flag = true means the product is not in the cart
...
router.post("/google", async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      const savedUser = await newUser.save();
      const token = jwt.sign({ id: savedUser._id }, secretKey); // Use savedUser._id instead of user._id
      const { password: pass, ...rest } = savedUser._doc; // Use savedUser instead of user
      res
        .cookie("access_token", token, { httpOnly: true })
        .status(200)
        .json(rest);
